...
func JsonInBlacklist(jwtList model.JwtBlacklist) (err error) {
	err = global.DEMO_DB.Create(&jwtList).Error
	return
}
...
func IsBlacklist(jwt string) bool {
	isNotFound := global.DEMO_DB.Where("jwt = ?", jwt).First(&model.JwtBlacklist{}).RecordNotFound()
	return !isNotFound
}
...
func GetRedisJWT(userName string) (err error, redisJWT string) {
	redisJWT, err = global.DEMO_REDIS.Get(userName).Result()
	return err, redisJWT
}
...
func SetRedisJWT(jwt string, userName string) (err error) {
	...
	err = global.DEMO_REDIS.Set(userName, jwt, timer).Err()
	return err
}
